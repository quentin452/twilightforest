...
public class TFTreasure {
    ...
    public boolean generate(World world, Random rand, int cx, int cy, int cz, Block chestBlock) {
        ...
        for (int i = 0; i < 2; i++) {
            flag &= addItemToChest(world, treasureRNG, cx, cy, cz, getUncommonItem(treasureRNG));
        }
    ...
    /**
     * Gets a common item. If the useless list is not empty, there is a 1/4 chance we get a useless item instead.
     */
    ...
    /**
     * Fills the table with items for the specified type. This should be refactored into seperate functions someday.
     */
