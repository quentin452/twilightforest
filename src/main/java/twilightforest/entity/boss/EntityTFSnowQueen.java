...
public class EntityTFSnowQueen extends EntityMob implements IBossDisplayData, IEntityMultiPart, IBreathAttacker {
    ...
    public void onDeath(DamageSource par1DamageSource) {
        ...
        if (par1DamageSource.getEntity() instanceof EntityPlayer) {
            ...
            ((EntityPlayer) par1DamageSource.getEntity()).triggerAchievement(TFAchievementPage.twilightProgressGlacier);
    ...
    private float getIceShieldAngle(int i) {
        // System.out.println("Getting angle for shield " + i + " and it is " + (((float)Math.PI / 3F) *
        // (float)i));
