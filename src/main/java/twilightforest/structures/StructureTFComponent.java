...
//	}
    ...
    protected int getMetadataAtCurrentPosition(World world, int x, int y, int z, StructureBoundingBox structureBoundingBox) {
        int l = this.getXWithOffset(x, z);
        int i1 = this.getYWithOffset(y);
        int j1 = this.getZWithOffset(x, z);
        return !structureBoundingBox.isVecInside(l, i1, j1) ? 0 : world.getBlockMetadata(l, i1, j1);
    }

    /**
     * current Position depends on currently set Coordinates mode, is computed here
     */
    protected void setTileEntityAtCurrentPosition(World world, TileEntity te, int x, int y, int z, StructureBoundingBox sbb) {
        int i1 = this.getXWithOffset(x, z);
        int j1 = this.getYWithOffset(y);
        int k1 = this.getZWithOffset(x, z);

        if (sbb.isVecInside(i1, j1, k1)) {
            world.setTileEntity(i1, j1, k1, te);
        }
    }
     ...
     * current Position depends on currently set Coordinates mode, is computed here
     */
    protected void placeBlockWithTileEntityAtCurrentPosition(World par1World, Block par2, int par3, TileEntity te, int par4, int par5, int par6, StructureBoundingBox par7StructureBoundingBox) {
    ...
    protected void fillWithTileEntities(World world, StructureBoundingBox sbb, int minX, int minY, int minZ, int maxX, int maxY, int maxZ, TileEntity tileEntity) {
    ...
    protected void partiallyFillWithMetadataBlocks(World world, StructureBoundingBox structureBoundingBox, int minX, int minY, int minZ, int maxX, int maxY, int maxZ, Block placeBlock, int placeBlockMeta, Random random, float placeRate) {
        for (int i2 = minY; i2 <= maxY; ++i2) {
            for (int j2 = minX; j2 <= maxX; ++j2) {
                for (int k2 = minZ; k2 <= maxZ; ++k2) {
                    if (random.nextFloat() <= placeRate) {
                        this.placeBlockAtCurrentPosition(world, placeBlock, placeBlockMeta, j2, i2, k2, structureBoundingBox);
                    }
                }
            }
        }
    ...
    protected void fillWithTileEntityMetadataBlocks(World world, StructureBoundingBox sbb, int minX, int minY, int minZ, int maxX, int maxY, int maxZ, Block placeBlock, int placeBlockMeta, TileEntity placeTileEntity, Block replaceBlock, int replaceBlockMeta, boolean onlyReplace) {
    	fillWithMetadataBlocks(world, sbb, minX, minY, minZ, maxX, maxY, maxZ, placeBlock, placeBlockMeta, replaceBlock, replaceBlockMeta, onlyReplace);
