...
public class ComponentTFNagaCourtyardTerraceAbstract extends ComponentTFNagaCourtyardRotatedAbstract {
    ...
    public boolean addComponentParts(World world, Random random, StructureBoundingBox structureBoundingBox) {
    	...
    	for(int i = 0; i < 17; i++) {
        	for(int j = 0; j < 17; j++) {
        		if(this.getBlockAtCurrentPosition(world, i, 1, j, structureBoundingBox) == Blocks.air)
        			groundAir++;
        		if(this.getBlockAtCurrentPosition(world, i, 0, j, structureBoundingBox) == Blocks.air)
        			undergroundAir++;
        	}
    	}
    	...
    	this.fillWithBlocks(world, structureBoundingBox, 0, 1, 0, 16, 1, 16, Blocks.stonebrick, Blocks.stonebrick, true);
    	this.fillWithMetadataBlocks(world, structureBoundingBox, 1, 1, 1, 15, 1, 15, Blocks.stone_slab, 5, Blocks.stone_slab, 5, true);
    	...
    	for(int i = 0; i < 6; i++) {
        	for(int j = 0; j < 6; j++) {
        		if(!((i == 2 || i == 3) && (j == 2 || j == 3))) {
            		int startX = i * 3;
            		int startZ = j * 3;
                	this.fillWithBlocks(world, structureBoundingBox, startX, 0, startZ, startX + 1, 0, startZ + 1, TFBlocks.nagastoneEtched, TFBlocks.nagastoneEtched, true);
        		}
        	}
    	}
