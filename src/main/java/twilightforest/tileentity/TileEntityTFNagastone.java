...
public class TileEntityTFNagastone extends TileEntity {
	...
	public enum ConnectionType { NO, D, U, E, W, N, S, DE, DW, DN, DS, UE, UW, UN, US, UD, EW, EN, ES, WN, WS, NS, triD, triU, triE, triW, triN, triS, CROSS};
	...
	public int UpdateNeighbourCount() {
		neighbourCount = (neighbourDown? 1 : 0) + (neighbourUp? 1 : 0) + (neighbourEast? 1 : 0) + (neighbourWest? 1 : 0) + (neighbourNorth? 1 : 0) + (neighbourSouth? 1 : 0);
	...
	public void readFromNBT(NBTTagCompound par1NBTTagCompound) {
		...
		if(par1NBTTagCompound.hasKey("Facing"))
			facing = GetFacing(par1NBTTagCompound.getInteger("Facing"));
		if(par1NBTTagCompound.hasKey("NeighDown"))
			neighbourDown = par1NBTTagCompound.getBoolean("NeighDown");
		if(par1NBTTagCompound.hasKey("NeighUp"))
			neighbourUp = par1NBTTagCompound.getBoolean("NeighUp");
		if(par1NBTTagCompound.hasKey("NeighEast"))
			neighbourEast = par1NBTTagCompound.getBoolean("NeighEast");
		if(par1NBTTagCompound.hasKey("NeighWest"))
			neighbourWest = par1NBTTagCompound.getBoolean("NeighWest");
		if(par1NBTTagCompound.hasKey("NeighNorth"))
			neighbourNorth = par1NBTTagCompound.getBoolean("NeighNorth");
		if(par1NBTTagCompound.hasKey("NeighSouth"))
			neighbourSouth = par1NBTTagCompound.getBoolean("NeighSouth");
		if(par1NBTTagCompound.hasKey("IsOutdated"))
			isOutdated = par1NBTTagCompound.getBoolean("IsOutdated");
		...
		/*if(isOutdated) {
        	...
        	case 14: {
        		...
        		if(neighbour == null) 
        			neighbour = (TileEntityTFNagastone)worldObj.getTileEntity(xCoord, yCoord + 1, zCoord);
    			if(neighbour == null)
    				this.facing = Facing.NORTH;
    			else
    				this.facing = neighbour.facing;
        		break;
        	}
        	case 15: {
        		worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, 1, 2);
        		this.direction = Direction.SIDE;
        		TileEntityTFNagastone neighbourZminus = (TileEntityTFNagastone)worldObj.getTileEntity(xCoord, yCoord, zCoord - 1);
        		TileEntityTFNagastone neighbourZplus = (TileEntityTFNagastone)worldObj.getTileEntity(xCoord, yCoord, zCoord + 1);
